...
create index if not exists idx_leads_workspace_created_id on public.leads(workspace_id, created_at desc, id);
create index if not exists idx_clients_workspace_created_id on public.clients(workspace_id, created_at desc, id);
create index if not exists idx_projects_workspace_created_id on public.projects(workspace_id, created_at desc, id);
create index if not exists idx_tasks_workspace_created_id on public.tasks(workspace_id, created_at desc, id);
create index if not exists idx_calendar_workspace_date_id on public.calendar_events(workspace_id, date_value, id);
create index if not exists idx_approvals_workspace_created_id on public.approval_requests(workspace_id, created_at desc, id);
create index if not exists idx_messages_workspace_timestamp_id on public.messages(workspace_id, timestamp_value, id);
create index if not exists idx_communications_workspace_timestamp_id on public.communications(workspace_id, timestamp_value, id);
create index if not exists idx_timeline_workspace_timestamp_id on public.timeline_events(workspace_id, timestamp_value desc, id);
...
-- Fotos de perfil ficam em um bucket próprio. O caminho é sempre
-- {auth.uid()}/profile, então não é necessário salvar a imagem em profiles.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('studiodesk-avatars', 'studiodesk-avatars', true, 5242880,
...
begin
  ...
  create policy "studiodesk_avatar_select_own" on storage.objects
    for select to authenticated
    using (bucket_id = 'studiodesk-avatars' and (storage.foldername(name))[1] = auth.uid()::text);

  drop policy if exists "studiodesk_avatar_insert_own" on storage.objects;
  create policy "studiodesk_avatar_insert_own" on storage.objects
    for insert to authenticated
    with check (bucket_id = 'studiodesk-avatars' and (storage.foldername(name))[1] = auth.uid()::text);

  drop policy if exists "studiodesk_avatar_update_own" on storage.objects;
  create policy "studiodesk_avatar_update_own" on storage.objects
    for update to authenticated
    using (bucket_id = 'studiodesk-avatars' and (storage.foldername(name))[1] = auth.uid()::text)
    with check (bucket_id = 'studiodesk-avatars' and (storage.foldername(name))[1] = auth.uid()::text);

  drop policy if exists "studiodesk_avatar_delete_own" on storage.objects;
  create policy "studiodesk_avatar_delete_own" on storage.objects
