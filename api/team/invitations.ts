...
export default async function handler(request: any, response: any) {
  ...
  try {
    ...
    if (resource === 'members' && (request.method === 'GET' || request.method === 'DELETE')) {
      ...
      if (request.method === 'DELETE') {
        if (actorMembership.role !== 'admin') return response.status(403).json({ error: 'Somente administradores podem remover membros.' });
      ...
      const { data: memberships, error: membershipsError } = await admin.from('workspace_members')
        .select('user_id,role,created_at').eq('workspace_id', actorMembership.workspace_id).order('created_at');
      ...
      return response.status(200).json({ workspace: { id: workspace.id, name: workspace.name, plan: workspace.plan }, currentUserRole: actorMembership.role, members });
    }
    ...
    if (/remove_workspace_member|function .* does not exist/i.test(message)) {
