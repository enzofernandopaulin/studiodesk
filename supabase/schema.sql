...
begin
  ...
  create policy "studiodesk_avatar_select_own" on storage.objects
    ...
    using (bucket_id = 'studiodesk-avatars' and split_part(name, '/', 1) = (select auth.uid())::text);
  ...
  create policy "studiodesk_avatar_insert_own" on storage.objects
    ...
    with check (bucket_id = 'studiodesk-avatars' and split_part(name, '/', 1) = (select auth.uid())::text);
