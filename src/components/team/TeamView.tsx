...
export const TeamView: React.FC = () => {
  ...
  const requestMemberRemoval = async (member: (typeof team)[number]) => {
    const confirmed = window.confirm(
      `Excluir ${member.name} do workspace ${user.companyName}?\n\n` +
      'O colaborador perderá o acesso e seu vínculo será removido do banco de dados desta equipe.\n\n' +
      'Clique em OK para confirmar ou em Cancelar para manter o membro.'
    );
    if (!confirmed) return;
          ...
          >
            ...
            <div className="pt-3 border-t border-[#DDE3E8] flex items-center justify-between">
              ...
              {user.role === 'admin' && member.userId !== user.id && !member.isOwner && (
                <button
                  onClick={() => void requestMemberRemoval(member)}
                  disabled={removingMemberId === member.id}
                  className="text-xs text-rose-600 hover:text-rose-800 font-semibold disabled:cursor-not-allowed disabled:text-rose-300"
                >
